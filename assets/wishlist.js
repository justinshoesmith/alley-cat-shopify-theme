...
if (MinimogSettings.template === "page.wishlist") {
  const wrapper = document.querySelector(".m-wishlist-page-content__wrapper");
  const wishlist = MinimogTheme.Wishlist;
  if (wrapper && wishlist) {
    let reconciled = false;
    const reconcile = async () => {
      if (reconciled || !wrapper.classList.contains("is-visible")) return;
      reconciled = true;
      observer.disconnect();
      const missingCards = wishlist.products.filter((handle) => !wishlist.productNodes[handle]);
      if (!missingCards.length) return;
      const unavailable = await Promise.all(missingCards.map(async (handle) => {
        try {
          const response = await fetch(`/products/${encodeURIComponent(handle)}.js`);
          return response.status === 404 ? handle : null;
        } catch (_) {
          return null;
        }
      }));
      const gone = new Set(unavailable.filter(Boolean));
      if (!gone.size) return;
      wishlist.products = wishlist.products.filter((handle) => !gone.has(handle));
